...
router.get("/ledger", requireAuth, async (req: any, res) => {
  const userId = req.clerkUserId as string;
  const limit = Math.min(parseInt(String(req.query.limit ?? "50")), 200);
  try {
    const entries = await walletService.getLedger(userId, limit);
    res.json(entries);
  } catch {
    res.status(500).json({ error: "DB error" });
  }
...
router.post("/topup", requireAuth, async (req: any, res): Promise<void> => {
  ...
  try {
    ...
    notif.notifyWalletTopup(Number(amount), requestId);
